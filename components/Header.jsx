...
const Header = () => {
  ...
  return (
    <header className={clsx("fixed top-0 left-0   z-50  w-full py-10 transition-all duration-500 max-lg:py-3", hasScrolled && "py-2 bg-black-100 backdrop-blur-[8px]")}>
...
<div className={clsx("w-full max-lg:fixed max-lg:top-0 max-lg:left-0 max-lg:w-full max-lg:bg-s2 max-lg:opacity-0" , isOpen ? "max-lg:opacity-100" : "max-lg:pointer-events-none")}>
      ...
      <img src={`/images/${isOpen ? "close" : "magic"}.svg`} alt ="hemburger" className="size-1/2 object-contain" />
      </button>
      </div>
    </header>
  );
